/* A09, 6809 Assembler. 
   ...
   Parts (C) Copyright 2001-2005 H. Seib.
...
#define VERSION      "1.20"
#define VERSNUM      "$0114"            /* can be queried as &VERSION        */
...
#define OPTION_PAG    0x00000001L       /* page formatting ON                */
#define OPTION_CON    0x00000002L       /* print cond. skipped code          */
#define OPTION_MAC    0x00000004L       /* print macro calling line (default)*/
#define OPTION_EXP    0x00000008L       /* print macro expansion lines       */
#define OPTION_SYM    0x00000010L       /* print symbol table (default)      */
#define OPTION_MUL    0x00000020L       /* print multiple oc lines (default) */
#define OPTION_LP1    0x00000040L       /* print pass 1 listing              */
#define OPTION_DAT    0x00000080L       /* print date in listing (default)   */
#define OPTION_NUM    0x00000100L       /* print line numbers                */
#define OPTION_INV    0x00000200L       /* print invisible lines             */
#define OPTION_TSC    0x00000400L       /* TSC-compatible source format      */
#define OPTION_WAR    0x00000800L       /* print warnings                    */
#define OPTION_CLL    0x00001000L       /* check line length (default)       */
#define OPTION_LFN    0x00002000L       /* print long file names             */
#define OPTION_LLL    0x00004000L       /* list library lines                */
#define OPTION_GAS    0x00008000L       /* Gnu AS compatibility              */
#define OPTION_REL    0x00010000L       /* print relocation table            */
#define OPTION_H63    0x00020000L       /* HD6309 mode                       */
#define OPTION_TXT    0x00040000L       /* print text table                  */
#define OPTION_LIS    0x00080000L       /* print assembler output listing    */
...
char *errormsg[]=
  ...
  "Illegal addressing mode",            /* 2                                 */
  "Undefined label",                    /* 4                                 */
  "Multiple definitions of label",      /* 8                                 */
  "Relative branch out of range",       /* 16                                */
  "Missing label",                      /* 32                                */
  "Unknown option specified",           /* 64                                */
  "Out of memory",                      /* 128                               */
  "Nesting not allowed",                /* 256                               */
  "Illegal for current relocation mode",/* 512                               */
  "",                                   /* 1024                              */
  "",                                   /* 2048                              */
  "",                                   /* 4096                              */
  "",                                   /* 8192                              */
  NULL,                                 /* reserved for ERR directive!       */
  "Illegal mnemonic"                    /* 32768                             */
  };
...
char *warningmsg[] =
  {
  "Long branch within short branch "    /* 1                                 */
      "range could be optimized",
  "Symbolic text undefined",            /* 2                                 */
  "Area already in use",                /* 4                                 */
  "Ambiguous 6800 alternate notation",  /* 8                                 */
  "",                                   /* 16                                */
  "",                                   /* 32                                */
  "",                                   /* 64                                */
  "",                                   /* 128                               */
...
#define ADRMODE_POST 3                  /* 3 = postbyte                      */
#define ADRMODE_PCR  4                  /* 4 = PC relative (with postbyte)   */
#define ADRMODE_IND  5                  /* 5 = indirect                      */
#define ADRMODE_PIN  6                  /* 6 = PC relative & indirect        */
...
char condline = 0; /* flag whether this is a conditional line */
...
fprintf(listfile,
        ...
        (dwOptions & OPTION_H63) ? 6309 : 6809,
        nCurPage);
...
if (insert)                             /* if inserting,                     */
  ...
  if (lclcounter == MAXLABELS)
    {
    printf("%s(%d): error 25: out of local symbol storage\n",
...
if (s)                                  /* if symbol not found               */
  ...
  if (symcounter == MAXLABELS)
    {
    printf("%s(%d): error 23: out of symbol storage\n",
           expandfn(curline->fn), curline->ln);
    if (((dwOptions & OPTION_LP1) || pass == 2) && (listing & LIST_ON))
...
void scanoperands(struct relocrecord *pp)
{
char c, *oldsrcptr;
...
if ((warning & WRN_OPT) &&              /* excessive branch, TSC style       */
...
for (i = 0; szOpt[i] && i < sizeof(szOpt); i++)
...
if ((listing & LIST_ON) &&              /* if listing pass 1                 */
  ...
  outlist();                            /* show macro invocation BEFORE      */
...
printf("srcname ............ source file name\n");
...
if (argc != 2)                          /* if not exactly one filename left  */
...
if (suppress)
  {
  printf("%s(%d) : error 18: improperly nested IF statements\n",
...
if (common)
  {
  printf("%s(%d) : error 24: improperly nested COMMON statements\n",
         expandfn(plast->fn), plast->ln);
  if (((dwOptions & OPTION_LP1) || pass == 2) &&
      (listing & LIST_ON))
...
readfile(srcname, 0, NULL);             /* read in the source file           */
...
if (errors)
  {
  printf("%d error(s) in pass 1\n",errors);
  if ((listing & LIST_ON) && (dwOptions & OPTION_LP1))
    putlist( "%d error(s) in pass 1.\n", errors);
  nTotErrors = errors;
  }
if (warnings)
  {
  printf("%d warning(s) in pass 1\n", warnings);
  if ((listing & LIST_ON) && (dwOptions & OPTION_LP1))
    putlist(
            "%d warning(s) in pass 1.\n", warnings);
...
if (listing & LIST_ON)
  ...
  if ((dwOptions & OPTION_LP1))
    ...
    putlist( "%d error(s), %d warning(s) unlisted in pass 1\n",
...
if (errors)
  {
  printf("%d error(s) in pass 2\n", errors);
  nTotErrors += errors;
  }
if (warnings)
  {
  printf("%d warning(s) in pass 2\n", warnings);
  nTotWarnings += warnings;
  }
...
if (listing & LIST_ON)
  ...
  if (errors || warnings)
    putlist("\n");
  if (errors)
    putlist( "%d error(s) in pass 2.\n", errors);
  if (warnings)
    putlist( "%d warning(s) in pass 2.\n", warnings);
